...
const TOURNAMENT_FORMATS={
 ...
 ,swiss:{name:"Sistema svizzero + playoff",tag:"8 partite garantite",description:"Classifica unica e turni contro avversari con rendimento simile, evitando le rivincite quando possibile."}
};
const TOURNAMENT_FORMATS_EN={
 ...
 ,swiss:{name:"Swiss system + playoffs",tag:"8 guaranteed matches",description:"One overall table with pairings between similarly ranked teams, avoiding rematches whenever possible."}
};

function blankState(){
  return {version:3,id:uid("tournament"),experience:"complete",mode:"single",teamSize:2,tournamentFormat:"classic",swissRounds:8,swissQualifiers:16,swissWarnings:[],title:"Nuovo torneo",tables:4,availableHours:6,matchMinutes:12,seed:"BALILLA-2026",nameTheme:"sportive",entryFee:0,fixedCosts:0,prizeSplit:[50,30,20],withdrawalPolicy:"forfeit",players:[],teams:[],groups:[],matches:[],knockout:[],createdAt:new Date().toISOString(),log:[]};
}
let library=emptyLibrary(), state=null, currentView="dashboard", matchFilter="all";
const $=s=>document.querySelector(s), main=$("#main");

function emptyLibrary(){return{tournaments:[],currentId:null}}
function loadLibrary(userId){if(!userId)return emptyLibrary();try{const x=JSON.parse(localStorage.getItem(AccountIsolation.key(userId))||"null");if(x?.tournaments)return{tournaments:AccountIsolation.filterOwned(x.tournaments,userId),currentId:x.currentId}}catch{}return emptyLibrary()}
function persistLibrary(){const userId=remoteSession?.user?.id;if(!userId)return;library.tournaments=AccountIsolation.filterOwned(library.tournaments,userId);localStorage.setItem(AccountIsolation.key(userId),JSON.stringify(library))}
function resetUserRuntime(userId,{clearStorage=false}={}){state=null;library=emptyLibrary();currentView="dashboard";matchFilter="all";syncInFlight=0;if(clearStorage){AccountIsolation.clearUserStorage(localStorage,userId);AccountIsolation.clearUserStorage(sessionStorage,userId)}}
function save(msg){if(!state)return;const userId=remoteSession?.user?.id;if(ONLINE&&!userId)return showAuth("Sessione scaduta: accedi nuovamente.");if(userId){if(state.ownerId&&state.ownerId!==userId)return toast("Operazione bloccata: torneo appartenente a un altro account");state.ownerId=userId}if(msg){state.log.unshift({at:new Date().toISOString(),text:msg});state.log=state.log.slice(0,30)}const i=library.tournaments.findIndex(t=>t.id===state.id);if(i>=0)library.tournaments[i]=state;else library.tournaments.push(state);library.currentId=state.id;persistLibrary();if(remoteSession){syncInFlight++;updateSaveState();syncTournament(state).then(()=>toast("Sincronizzato online")).catch(()=>toast("Salvato sul dispositivo · sincronizzazione da riprovare")).finally(()=>{syncInFlight--;updateSaveState()})}else toast("Salvato sul dispositivo")}
...
async function syncTournament(t){
 const userId=remoteSession?.user?.id;if(!userId)throw Error("Sessione non valida");if(t.ownerId&&t.ownerId!==userId)throw Error("Proprietario torneo non valido");t.ownerId=userId;
 ...
 if(t.remoteId){await supabase(`/rest/v1/tournaments?id=eq.${encodeURIComponent(t.remoteId)}&${AccountIsolation.ownerQuery(userId)}`,{method:"PATCH",headers:{"Prefer":"return=minimal"},body:JSON.stringify(payload)})}
...
async function deleteRemoteTournament(t){const userId=remoteSession?.user?.id;if(!t?.remoteId||!userId)return;if(!AccountIsolation.owns(t,userId))throw Error("Proprietario torneo non valido");await supabase(`/rest/v1/tournaments?id=eq.${encodeURIComponent(t.remoteId)}&${AccountIsolation.ownerQuery(userId)}`,{method:"DELETE",headers:{"Prefer":"return=minimal"}})}
async function loadRemoteLibrary(){
 const userId=remoteSession?.user?.id;if(!userId)throw Error("Sessione non valida");library=loadLibrary(userId);const rows=await supabase(`/rest/v1/tournaments?${AccountIsolation.ownerQuery(userId)}&select=id,owner_id,slug,title,data,created_at&order=created_at.desc`);
 const remote=rows.filter(r=>r.owner_id===userId).map(r=>Object.assign(blankState(),r.data,{remoteId:r.id,slug:r.slug,title:r.title,ownerId:r.owner_id})),local=AccountIsolation.filterOwned(library.tournaments,userId),eligibleLocal=local.filter(t=>!t.remoteId),merged=[...remote];
...
async function restoreSession(){try{const s=JSON.parse(localStorage.getItem(SESSION_KEY)||"null");if(!s)return null;if(s.expires_at&&s.expires_at*1000>Date.now()+60000)return s;if(!s.refresh_token){localStorage.removeItem(SESSION_KEY);return null}const r=await fetch(`${SUPABASE_URL}/auth/v1/token?grant_type=refresh_token`,{method:"POST",headers:{"apikey":SUPABASE_KEY,"Content-Type":"application/json"},body:JSON.stringify({refresh_token:s.refresh_token})});if(!r.ok){localStorage.removeItem(SESSION_KEY);return null}const fresh=await r.json();saveSession(fresh);return fresh}catch{localStorage.removeItem(SESSION_KEY);return null}}
...
function createTournament(experience=null){
 ...
 const initialFormat=$("#newTournamentForm select[name='tournamentFormat']");if(initialFormat)initialFormat.insertAdjacentHTML("afterbegin",'<option value="swiss">Sistema svizzero + playoff</option>');
}
function dashboard(){
...
function estimateTournament(teamCount,format=state?.tournamentFormat||"classic"){
 ...
 if(format==="swiss"){const cfg=SwissEngine.clampConfig(teamCount,state?.swissRounds||8,state?.swissQualifiers||16);groups=1;groupMatches=Math.floor(teamCount/2)*cfg.rounds;ko=cfg.qualifiers;guaranteed=cfg.rounds}
...
function format(){
 ...
 ${current==="swiss"?swissConfigPanel():""}<div class="card format-summary"><h2>Configurazione usata per la stima</h2><p><b>${count} squadre</b> · ${state.tables||4} biliardini · ${state.availableHours||6} ore disponibili · ${state.matchMinutes||12} minuti medi per partita.</p><p class="muted">La scelta può essere cambiata finché non sono stati inseriti risultati. Cambiandola, gironi, calendario e tabellone vengono rigenerati.</p></div>`+
 stepNav("teams","groups","Avanti: genera la struttura",count<2,(uiLanguage==="en"?"Selected: ":"Hai scelto ")+formats[current].name);
}

function groups(){
 const mode=state.tournamentFormat||"classic",est=estimateTournament(state.teams.length),over=est.minutes>(state.availableHours||6)*60,direct=mode==="knockout",swiss=mode==="swiss",leagueMode=mode==="league"||mode==="league_playoff",minTeams=direct?2:4;
 const title=direct?"Eliminazione diretta":swiss?"Sistema svizzero":leagueMode?"Campionato":"Gironi";
 const sub=direct?`${state.teams.length} squadre · tabellone con passaggi automatici.`:swiss?`${state.teams.length} squadre · ${state.swissRounds||8} turni · migliori ${state.swissQualifiers||16} ai playoff.`:leagueMode?`Girone unico · ${state.teams.length} squadre · ${state.teams.length?state.teams.length-1:0} partite garantite.`:`${est.groups||0} gironi dinamici · ${state.teams.length} squadre · fase finale prevista da ${est.ko}.`;
...
function matches(){
 const swiss=state.tournamentFormat==="swiss",filtered=state.matches.filter(m=>matchFilter==="all"||(swiss?String(m.swissRound)===matchFilter:String(m.group)===matchFilter)),leagueMode=["league","league_playoff"].includes(state.tournamentFormat),direct=state.tournamentFormat==="knockout",currentRound=Math.max(0,...state.matches.map(m=>m.swissRound||0)),roundComplete=!swiss||state.matches.filter(m=>m.swissRound===currentRound).every(m=>m.played),canNext=swiss&&currentRound<(state.swissRounds||8)&&roundComplete;
 main.innerHTML=pageHead("Fase 5","Calendario e risultati",direct?"Le partite sono gestite direttamente nel tabellone finale.":swiss?`Turno svizzero ${currentRound} di ${state.swissRounds||8}. Gli abbinamenti successivi usano la classifica aggiornata.`:`Partite distribuite su ${state.tables||4} biliardini senza sovrapposizioni.`,`<button class="btn secondary test-btn" data-action="random-group-results" ${state.matches.length?"":"disabled"}>⚗ Risultati casuali</button>${canNext?`<button class="btn" data-action="next-swiss-round">Genera turno ${currentRound+1}</button>`:""}<button class="btn secondary" data-action="print">Stampa</button>`)+
 `<div class="tabs"><button data-filter="all" class="${matchFilter==="all"?"active":""}">Tutte</button>${swiss?Array.from({length:currentRound},(_,i)=>`<button data-filter="${i+1}" class="${matchFilter==i+1?"active":""}">Turno ${i+1}</button>`).join(""):state.groups.map((_,i)=>`<button data-filter="${i}" class="${matchFilter==i?"active":""}">${leagueMode?"Campionato":`Girone ${groupLabel(i)}`}</button>`).join("")}</div>
 ...
 stepNav("groups","knockout",state.tournamentFormat==="league"?"Avanti: proclama il vincitore":"Avanti: fase finale",!direct&&(!state.matches.length||state.matches.some(m=>!m.played)||(swiss&&currentRound<(state.swissRounds||8))),swiss&&currentRound<(state.swissRounds||8)?"Completa e genera tutti i turni svizzeri":"Completa tutte le partite per proseguire");
}
function matchRow(m){if(m.bye)return `<div class="match-row swiss-bye"><span class="badge gold">Turno ${m.swissRound}</span><span style="text-align:right"><b>${esc(nameTeam(m.a))}</b></span><span class="match-score">BYE</span><span>3 punti automatici</span><span class="badge gray">Riposo</span></div>`;return `<div class="match-row"><span class="badge gray">T${m.slot} · Tav. ${m.table}</span><span style="text-align:right"><b>${esc(nameTeam(m.a))}</b></span><span class="match-score ${m.played?"":"pending"}">${m.played?`${m.ga} — ${m.gb}`:"—"}</span><span><b>${esc(nameTeam(m.b))}</b></span><button class="btn small ${m.played?"secondary":""}" data-result="${m.id}">${m.played?"Modifica":"Risultato"}</button></div>`}

function knockout(){
 const mode=state.tournamentFormat||"classic",leagueOnly=mode==="league",direct=mode==="knockout";
 const can=direct?state.teams.length>=2:mode==="swiss"?Math.max(0,...state.matches.map(m=>m.swissRound||0))>=(state.swissRounds||8)&&state.matches.every(m=>m.played):state.groups.length&&state.groups.every((g,i)=>state.matches.filter(m=>m.group===i).every(m=>m.played));
 const champ=getChampion();
 main.innerHTML=pageHead("Fase 6",leagueOnly?"Vincitore del campionato":"Fase finale",leagueOnly?"Il primo classificato al termine di tutte le partite vince il torneo.":`Tabellone ${direct?"a eliminazione diretta":`da ${mode==="swiss"?(state.swissQualifiers||16):mode==="league_playoff"?playoffSize(state.teams.length):suggestedKnockoutSize(state.teams.length)} squadre`}.`,`${state.knockout.length?`<button class="btn secondary test-btn" data-action="random-knockout">⚗ Completa casualmente</button>`:""}<button class="btn secondary" data-action="print">Stampa</button>${!leagueOnly&&!state.knockout.length?`<button class="btn" data-action="generate-knockout" ${can?"":"disabled"}>Crea tabellone</button>`:""}`)+
...
function rules(){
 ...
 if(uiLanguage==="en"){
  ...
  const qualification=formatKey==="league"?"The single league has no qualification stage: the top-ranked team wins.":formatKey==="knockout"?"All teams enter the bracket directly; any byes are assigned by the initial draw.":formatKey==="swiss"?`After ${state.swissRounds||8} Swiss rounds, the top ${state.swissQualifiers||16} teams qualify. Pairings use similar scores and avoid rematches whenever mathematically possible. An odd field receives one rotating bye worth 3 points.`:formatKey==="league_playoff"?"At the end of the league, the best 4 or 8 teams qualify, depending on the number of participants. Pairings follow the standings: first versus the lowest qualifier, second versus the second-lowest, and so on.":"The knockout-stage size is chosen automatically in proportion to the number of participants. Group winners qualify first, followed by runners-up, third-placed teams, and so on. If groups have different sizes, teams are compared using points, goal difference and goals scored per match.";
  ...
  <h2>5. Standings and tie-breakers</h2><p>${formatKey==="swiss"?"Swiss ranking: 1) points; 2) Buchholz score, equal to the sum of opponents’ points; 3) goal difference; 4) goals scored; 5) stable system draw.":"Teams are ranked by: 1) points; 2) goal difference; 3) goals scored; 4) a mini-table using the head-to-head matches among all teams that are still tied; 5) a stable system draw."}</p>
 ...
 <h2>5. Classifica e spareggi</h2><p>${formatKey==="swiss"?"Classifica svizzera: 1) punti; 2) Buchholz, cioè somma dei punti ottenuti dagli avversari affrontati; 3) differenza reti; 4) gol fatti; 5) sorteggio stabile del sistema.":"L’ordine è determinato da: 1) punti; 2) differenza reti; 3) gol fatti; 4) mini-classifica degli scontri tra tutte le squadre ancora a pari merito; 5) sorteggio stabile del sistema."}</p>
 <h2>6. Qualificazione</h2><p>${state.tournamentFormat==="league"?"Il girone unico non prevede qualificazioni: vince la prima classificata.":state.tournamentFormat==="knockout"?"Tutte le squadre entrano direttamente nel tabellone; gli eventuali passaggi automatici sono assegnati dal sorteggio iniziale.":state.tournamentFormat==="swiss"?`Dopo ${state.swissRounds||8} turni svizzeri si qualificano le migliori ${state.swissQualifiers||16}. Gli abbinamenti avvicinano squadre con punteggio simile ed evitano le rivincite quando matematicamente possibile. Con squadre dispari viene assegnato a rotazione un bye da 3 punti.`:state.tournamentFormat==="league_playoff"?"Al termine del campionato si qualificano le migliori 4 o 8 squadre, secondo il numero di partecipanti. Gli accoppiamenti seguono il piazzamento: prima contro ultima qualificata, seconda contro penultima e così via.":"La dimensione della fase finale è scelta automaticamente in proporzione ai partecipanti. Si qualificano prima le migliori classificate di ogni girone, poi le seconde, le terze e così via. Se i gironi hanno dimensioni diverse, il confronto usa punti, differenza reti e gol fatti per partita."}</p>
...
function swissStandings(){return SwissEngine.standings(state.teams.map(t=>t.id),state.matches,id=>seeded(state.seed+id)())}
function swissStandingsCard(){const rows=swissStandings(),played=Math.max(0,...state.matches.map(m=>m.swissRound||0)),qual=state.swissQualifiers||16;return `<article class="card"><div class="group-title"><b>Classifica generale svizzera</b><span>${played}/${state.swissRounds||8} turni generati</span></div>${(state.swissWarnings||[]).map(x=>`<div class="warning swiss-warning">${esc(x)}</div>`).join("")}<div class="table-wrap"><table><thead><tr><th>#</th><th>Squadra</th><th>Pt</th><th>Buchholz</th><th>DR</th><th>GF</th><th>Bye</th></tr></thead><tbody>${rows.map((s,i)=>`<tr class="${i<qual?"qualified-row":""}"><td>${i+1}</td><td><b>${esc(nameTeam(s.id))}</b></td><td>${s.pts}</td><td>${s.buchholz}</td><td>${s.gd>0?"+":""}${s.gd}</td><td>${s.gf}</td><td>${s.byes||0}</td></tr>`).join("")}</tbody></table></div></article>`}
function swissConfigPanel(){const cfg=SwissEngine.clampConfig(state.teams.length,state.swissRounds,state.swissQualifiers);return `<div class="card swiss-config"><h2>Configurazione sistema svizzero</h2><div class="form-grid"><label class="field">Turni svizzeri<input id="swissRoundsInput" type="number" min="1" max="${Math.max(1,state.teams.length-1)}" value="${cfg.rounds}"></label><label class="field">Squadre qualificate<select id="swissQualifiersInput">${[2,4,8,16,32,64,128].filter(n=>n<=state.teams.length).map(n=>`<option value="${n}" ${cfg.qualifiers===n?"selected":""}>${n}</option>`).join("")}</select></label></div><p class="muted">Ogni squadra disputa un incontro per turno. Con 24 squadre, la configurazione consigliata è 8 turni e 16 qualificate.</p><button class="btn secondary" data-action="save-swiss-config">Salva configurazione</button></div>`}
...
function generateGroups(){
 ...
 if(mode==="swiss"){const cfg=SwissEngine.clampConfig(ordered.length,state.swissRounds,state.swissQualifiers);state.swissRounds=cfg.rounds;state.swissQualifiers=cfg.qualifiers;state.groups=[ordered.map(t=>t.id)];state.matches=[];state.knockout=[];state.swissWarnings=[];scheduleSwissRound(1);save("Sistema svizzero inizializzato e primo turno generato");render();return}
...
function scheduleSwissRound(round){const ids=state.groups[0]||state.teams.map(t=>t.id),result=SwissEngine.pairRound(ids,state.matches,round,id=>seeded(state.seed+id)()),tableCount=Math.max(1,+state.tables||1),slotBase=Math.max(0,...state.matches.map(m=>m.slot||0));result.pairs.forEach((p,i)=>state.matches.push({id:uid("m"),group:0,swissRound:round,a:p.a,b:p.b,slot:slotBase+Math.floor(i/tableCount)+1,table:i%tableCount+1,played:false}));if(result.bye)state.matches.push({id:uid("m"),group:0,swissRound:round,a:result.bye,b:null,slot:slotBase+Math.ceil(result.pairs.length/tableCount),table:null,played:true,bye:true,ga:0,gb:0,note:"Bye: 3 punti automatici"});if(result.warning){state.swissWarnings=state.swissWarnings||[];state.swissWarnings.push(`Turno ${round}: ${result.warning}`)}matchFilter=String(round)}
function nextSwissRound(){const current=Math.max(0,...state.matches.map(m=>m.swissRound||0));if(state.tournamentFormat!=="swiss"||current>=(state.swissRounds||8))return;if(state.matches.some(m=>m.swissRound===current&&!m.played))return toast("Completa il turno corrente prima di generare il successivo");scheduleSwissRound(current+1);state.knockout=[];save(`Turno svizzero ${current+1} generato`);render()}
...
function qualifiers(){
 if(state.tournamentFormat==="swiss")return{all:swissStandings().slice(0,state.swissQualifiers||16).map((x,i)=>({...x,group:-1,seed:i+1}))};
...
document.addEventListener("click",e=>{
 ...
 const folder=e.target.closest("[data-open-tournament]");if(folder){const userId=remoteSession?.user?.id,candidate=library.tournaments.find(t=>t.id===folder.dataset.openTournament);if(!candidate||!AccountIsolation.owns(candidate,userId))return toast("Accesso al torneo non autorizzato");state=candidate;library.currentId=state.id;persistLibrary();currentView="dashboard";render();return}
 if(!b)return;
 ...
 if(b.dataset.action==="logout"){const userId=remoteSession?.user?.id;localStorage.removeItem(SESSION_KEY);resetUserRuntime(userId,{clearStorage:true});remoteSession=null;authMode="login";showAuth("Sessione terminata. I dati locali dell’account sono stati rimossi.");return}
 ...
 if(b.dataset.action==="next-swiss-round")nextSwissRound();
 if(b.dataset.action==="save-swiss-config"){const cfg=SwissEngine.clampConfig(state.teams.length,+$("#swissRoundsInput").value,+$("#swissQualifiersInput").value);if(state.matches.length&&!confirm("Modificare la configurazione azzera turni, risultati e playoff già generati. Continuare?"))return;state.swissRounds=cfg.rounds;state.swissQualifiers=cfg.qualifiers;state.groups=[];state.matches=[];state.knockout=[];state.swissWarnings=[];save("Configurazione sistema svizzero aggiornata");render();return}
...
function importRows(rows){
 ...
 if(state.mode==="single"){const ni=col("nome","nome e cognome","name"),ci=col("contatto","telefono","contact"),ri=col("ruolo","ruolo preferito","role"),li=col("livello","level");if(ni<0)throw Error("Colonna nome assente");if(ri<0||li<0)throw Error("Il file deve contenere le colonne Ruolo e Livello");for(const [offset,r] of data.entries()){const rowNumber=headerIndex+offset+2,name=cleanText(r[ni],LIMITS.name);if(!name)continue;if(!validName(name))throw Error(`Riga ${rowNumber}: nome non valido (${name})`);if(duplicateName(state.players,name))throw Error(`Riga ${rowNumber}: giocatore duplicato (${name})`);const role=parseImportedRole(r[ri]),level=parseImportedLevel(r[li]);if(!role)throw Error(`Riga ${rowNumber}: ruolo non valido. Usa Portiere, Attaccante o Indifferente`);if(!level)throw Error(`Riga ${rowNumber}: livello non valido. Usa Principiante, Intermedio o Esperto`);state.players.push({id:uid("p"),name,contact:cleanText(r[ci],LIMITS.contact),role,level})}}
...
function parseImportedRole(x=""){const v=String(x).trim().toLowerCase();return({portiere:"portiere",goalkeeper:"portiere",attaccante:"attaccante",forward:"attaccante",indifferente:"indifferente",either:"indifferente"})[v]||null}
function parseImportedLevel(x=""){const v=String(x).trim().toLowerCase();return({principiante:"principiante",beginner:"principiante",intermedio:"intermedio",intermediate:"intermedio",esperto:"esperto",expert:"esperto"})[v]||null}
function normalizeRole(x=""){return parseImportedRole(x)||"indifferente"}
function normalizeLevel(x=""){return parseImportedLevel(x)||"intermedio"}
