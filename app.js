...
function stepNav(previous,next,nextLabel="Avanti",disabled=false,hint=""){return `<div class="step-nav admin-only"><div>${previous?`<button class="btn secondary" data-view="${previous}">← Indietro</button>`:""}</div><div class="step-next">${hint?`<small>${hint}</small>`:""}<button class="btn" data-view="${next}" ${disabled?"disabled":""}>${nextLabel} →</button></div></div>`}
...
function createTournament(){
 modal(`<h2>Nuovo torneo</h2><form id="newTournamentForm" class="form-grid"><label class="field full">Nome del torneo<input name="title" maxlength="${LIMITS.title}" required autofocus placeholder="Es. Torneo Estate 2026"></label><label class="field">Biliardini disponibili<input name="tables" type="number" min="1" max="100" value="4" required></label><label class="field">Ore disponibili<input name="availableHours" type="number" min="1" max="240" step=".5" value="6" required></label><label class="field">Minuti per partita, cambio incluso<input name="matchMinutes" type="number" min="5" max="180" value="12" required></label><label class="field">Tipo iscrizione<select name="mode"><option value="single">Singoli</option><option value="team">Squadre</option></select></label><label class="field">Giocatori per squadra<select name="teamSize"><option value="2">2 giocatori</option><option value="3">3 giocatori</option></select></label><label class="field full">Formato iniziale<select name="tournamentFormat"><option value="classic">Gironi + playoff (consigliato)</option><option value="league">Girone unico</option><option value="knockout">Eliminazione diretta</option><option value="league_playoff">Campionato + playoff</option></select></label><div class="full"><button class="btn">Crea cartella</button></div></form>`,()=>{const d=Object.fromEntries(new FormData($("#newTournamentForm"))),title=cleanText(d.title,LIMITS.title);if(!title)return toast("Inserisci un nome valido per il torneo");state=blankState();state.title=title;state.tables=Math.min(100,Math.max(1,+d.tables||1));state.availableHours=Math.min(240,Math.max(1,+d.availableHours||6));state.matchMinutes=Math.min(180,Math.max(5,+d.matchMinutes||12));state.mode=d.mode;state.teamSize=+d.teamSize||2;state.tournamentFormat=d.tournamentFormat||"classic";save("Torneo creato");closeModal();currentView="registrations";render()});
}
function dashboard(){
...
function teams(){
 ...
 stepNav("registrations","format","Avanti: scegli il formato",state.teams.length<2,"Servono almeno 2 squadre");
}
