...
function showAuth(message=""){
 ...
 main.innerHTML=`<section class="auth-shell"><div class="card auth-card"><div class="brand-mark" style="margin-bottom:16px">⚽</div><div class="eyebrow">Foosball Tournament Manager</div><h1>${authMode==="login"?"Accedi":"Crea account"}</h1><p class="muted">${message||"Gestisci e pubblica i tuoi tornei da qualsiasi dispositivo."}</p><form id="authForm" class="form-grid"><label class="field full">Email<input name="email" type="email" autocomplete="email" required></label>${authMode==="signup"?`<label class="field full">Nome<input name="display_name" autocomplete="name" required></label>`:""}<label class="field full">Password<input name="password" type="password" minlength="8" autocomplete="${authMode==="login"?"current-password":"new-password"}" required></label><div class="full actions"><button class="btn">${authMode==="login"?"Accedi":"Registrati"}</button><button type="button" class="btn secondary" data-action="toggle-auth">${authMode==="login"?"Crea un account":"Ho già un account"}</button></div></form></div></section>`;applyUiMode();
}
